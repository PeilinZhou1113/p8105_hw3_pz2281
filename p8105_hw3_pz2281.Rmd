...
There are `r length(unique(instacart$aisle))` aisles and the aisle that most items are ordered from is `r most_ord_aisle`, which makes sense since most people often order vegetables online from instacart, amazon fresh etc.
...
Based on the table, we can see that on weekends(Saturday and Sunday) the 35-day record of the 63-year-old male tended to have lower activity counts. Typically, there were two Saturdays which the male seemed to have no activity at all, or maybe he had taken off the accelerometer on those days. And on Wednesday, the male normally had activity counts around 350000 to 450000.
